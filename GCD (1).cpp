...
int main()
{
     ...
     string s1, s2, s3; // объявляем строки
     cout << "Программа вычислает НОД трёх числе m, n, k > MaxInt! " << endl;
     cout << "Введите первое число: ";
     getline(cin, s1);
     // Проверяем все три строки, есть ли в них буквы и другие посторонние знаки, а также ограничиваем длину строки
     ...
     int len2 = s2.length();
     int len3 = s3.length();
     //добавляем нули слева к меньшим подстрокам
     if (len1 < len2 and len2 > len3)
     {
      ...
      // считаем сначала НОД(a,b), а затем НОД(НОД(a,b),c)
      longGcd(a, b);
      longGcd(a, c);
      
      cout << "НОД трёх чисел: " << a[part - 1];// Игнорируем ведущие нрли в начале
